...
| **D25** | **Never rename or delete the sync folder to test a failure.** Induce "folder unreachable" by pointing `syncDirUri` at a non-existent tree in `shared_prefs`, then restore it | The sync folder is Syncthing-managed (`.stfolder`, `.stversions`), so removing it does not simulate a local failure — it propagates the deletion to every peer. The prefs swap hits the same `safDir == null \|\| !safDir.isDirectory` guard while touching no synced data | [`06`](06_ROADMAP.md) 1.9 |
| **D26** | **Edit `shared_prefs` *before* `am force-stop`, never after** | `BackgroundService` is sticky and Android restarts the process within seconds, so a later edit is masked by the restarted process's cached values — a sync then reports `success=true` against a URI the file no longer contains, which reads exactly like the change silently failing | [`06`](06_ROADMAP.md) 1.9 |
| **D27** | **D14's premise no longer holds: the Windows machine now has a full Rust/NDK toolchain.** `rustc 1.98.0`, the `aarch64-linux-android` and `x86_64-linux-android` targets, and NDK `27.0.12077973` + `28.2.13676358` are all installed. `cargo check --workspace` and `cargo test` were run locally against `aw-server-rust@beta` on 2026-09-08 (76 tests passed) | D14 said builds are CI-only because the machine had no toolchain "deliberately". That reason is now factually wrong, so it should not keep steering work away from local verification. **CI-only for shipped APKs may still be the right policy** — reproducibility, and D23's fixed keystore — but that is a choice to re-make on its merits, not a limitation. Local `cargo check`/`cargo test` is now the fastest way to catch a broken merge before it reaches CI. Supersedes D14's rationale, not its conclusion | [`02`](02_ARCHITECTURE.md) §5 |
